...
public interface DErrcount_respository extends JpaRepository<DErrcount, Integer> {
	 ...
	 @Query(value = "SELECT a.* FROM d_errcount a  where a.codeStart <= ?2 and a.codeEnd>=?1 and temp1=?3 and dRecordCreationDate between ?4 and ?5   and groupNo in (select groupNo from d_groupset where head=?6) and workid=?7 order by codeStart,pageNum",nativeQuery=true)
