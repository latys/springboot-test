...
public interface DErrcount_respository extends JpaRepository<DErrcount, Integer> {
	 ...
	 @Query(value = "SELECT a.* FROM d_errcount a  where a.codeStart between ?1 and ?2 and temp1=?3 and dRecordCreationDate between ?4 and ?5  and workid=?7 and groupNo in (select groupNo from d_groupset where head=?6) order by codeStart,pageNum",nativeQuery=true)
	 public List<DErrcount> GetErrCountByDoubleGZ(Integer startCode,Integer endCode,String productType,Date startDate,Date endDate,int workid);
